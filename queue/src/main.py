...
@celery_app.task(
    name="tasks.download_anime", bind=True, max_retries=MAX_DOWNLOAD_RETRIES
)
def download_anime(
    self,
    anime_id: str,
    ...
    with SyncBrowser() as b:
        ...
        if len(download_links) == 0:
            logger.error(
                f"Error getting download link for {anime_id} - {episode_number}"
            )
            self.retry(countdown=RETRY_DOWNLOAD_INTERVAL)
        ...
        with DatabaseSession() as db:
            ...
            episode.status = "GETTING-FILE-LINK"
            db.add(episode)
        ...
        download_status = download_episode(
            ...
            episode_number,
            valid_download_link,
            download_info.server,
        )

        if not download_status:
            logger.error(f"Error downloading {anime_id} - {episode_number}")
            self.retry(countdown=RETRY_DOWNLOAD_INTERVAL)
